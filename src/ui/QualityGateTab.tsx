...
function statusTone(status: DeliverableReview["status"]): {
  bg: string;
  fg: string;
  label: string;
} {
...
function releaseTone(
  state: DeliverableReview["releaseDecision"]["approvalState"],
...
export function QualityGateTab({ context }: PluginDetailTabProps) {
  ...
  const reviewQuery = usePluginData<ReviewStatusData>("quality_gate.review", {
    issueId,
  });
  ...
  const releaseChip = review
    ? releaseTone(review.releaseDecision.approvalState)
    : releaseTone("pending");
  ...
  if (reviewQuery.loading && !reviewQuery.data) {
    return (
      <div style={{ padding: 24, color: "#cbd5e1", fontFamily: "system-ui" }}>
    ...
    >
      ...
      >
        <div>
          ...
          >
            Evidence-centric review cockpit
          </div>
          <h2 style={{ margin: "6px 0 8px", fontSize: 28, lineHeight: 1.1 }}>
            {issue?.title ?? `Issue ${issueId}`}
          </h2>
          <div style={{ color: "#94a3b8", fontSize: 13 }}>
            Issue ID: <code>{issueId}</code>
            {issue?.status ? ` · Host status: ${issue.status}` : ""}
          </div>
        </div>
      ...
      {!review ? (
        ...
        >
          <div style={panelStyle()}>
            ...
            <p style={{ color: "#94a3b8", fontSize: 14 }}>
              Turn this issue into an evidence bundle with a draft artifact,
              risk flags, and next-step guidance.
            </p>
            <div style={{ display: "grid", gap: 12 }}>
              <textarea
                ...
                rows={6}
                placeholder="Summarize the deliverable or current draft…"
              ...
              >
                <input
                  ...
                  onChange={(event) => setQualityScoreInput(event.target.value)}
                  placeholder="8"
                  style={textInputStyle()}
                />
                <input
                  ...
                  runAction(
                    () =>
                      submitAction({
                        ...
                        quality_score: Number(qualityScoreInput || 0),
          ...
          <div style={panelStyle()}>
            ...
            <ul style={{ color: "#cbd5e1", lineHeight: 1.7, paddingLeft: 18 }}>
              ...
              <li>
                Operator next-step template for revision, follow-up, or release
              </li>
            </ul>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          >
            <MetricCard
              label="Display score"
              value={`${review.qualityScore}/10`}
              hint={`Rendered score (${scoreColor(review.qualityScore)})`}
            />
            <MetricCard
              label="Decision score"
              value={`${review.decisionScore}/10`}
              hint="Thresholds operate on this score."
            />
            <MetricCard
              label="Confidence"
              value={`${review.draftArtifact.confidence}%`}
              hint={`${review.riskFlags.length} active risk flag(s)`}
            />
            <MetricCard
              label="Evidence hash"
              value={review.evidenceBundle.hash}
              hint="Stored in issue documents for audit."
            />
          </div>
          ...
          <div style={{ ...panelStyle(), display: "grid", gap: 14 }}>
            <div>
              ...
              <h3 style={{ margin: "6px 0" }}>
                {review.reviewSummary.headline}
              </h3>
              <div style={{ color: "#94a3b8", fontSize: 14 }}>
                {review.reviewSummary.disposition} ·{" "}
                {review.reviewSummary.reviewerHint}
              </div>
            </div>
            ...
            >
              <div style={{ display: "grid", gap: 12 }}>
                <div style={panelStyle()}>
                  ...
                  >
                    ...
                    <div
                      style={{
                        color: scoreColor(review.qualityScore),
                        fontWeight: 700,
                      }}
                    >
                      {review.draftArtifact.confidence}% confident
                    </div>
                  </div>
                  <div
                    style={{
                ...
                <div style={panelStyle()}>
                  ...
                  <div style={{ display: "grid", gap: 10 }}>
                    <textarea
                      ...
                      onChange={(event) => setOperatorNote(event.target.value)}
                      rows={4}
                      placeholder="Reviewer note for approvals or revisions…"
                      style={textInputStyle()}
                    />
                          ...
                          runAction(
                            () =>
                              rejectAction({
                                ...
                                comment:
                                  operatorNote ||
                                  "Revise this deliverable using the evidence package.",
                              }),
                            "Revision requested.",
                          )
                        }
                    ...
                    <button
                      ...
                      onClick={() =>
                        runAction(
                          ...
                          "Revision template refreshed.",
                        )
                      }
                      style={buttonStyle("#475569", busy)}
                    >
                      Refresh revision template
                    </button>
                    <button
                      disabled={busy}
                      onClick={() =>
                        runAction(
                          ...
                          "Release template refreshed.",
                        )
                      }
                      style={buttonStyle("#475569", busy)}
                    >
                      Refresh release template
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "grid", gap: 12 }}>
                ...
                <div style={panelStyle()}>
                  ...
                  <div style={{ display: "grid", gap: 10 }}>
                    {review.evidenceBundle.inputRefs
                      .concat(review.evidenceBundle.retrievedContext)
                      .map((ref) => (
                        <div
                          key={ref.id}
                          style={{
                ...
                <div style={panelStyle()}>
                  ...
                  <div
                    style={{ color: "#cbd5e1", fontSize: 13, lineHeight: 1.7 }}
                  >
                    <strong>Standards</strong>
                    <ul>
                      {review.evidenceBundle.standards.map((standard) => (
                        <li key={standard}>{standard}</li>
                      ))}
                    </ul>
                    <strong>Trace</strong>
                    <ul>
                      {review.evidenceBundle.trace.map((step) => (
          ...
          >
            <div style={panelStyle()}>
              ...
              <div style={{ display: "grid", gap: 10 }}>
                <input
                  value={returnAgent}
                  onChange={(event) => setReturnAgent(event.target.value)}
                  placeholder="Optional Paperclip agent ID…"
                  style={textInputStyle()}
                />
                <textarea
                  value={returnInstruction}
                  onChange={(event) => setReturnInstruction(event.target.value)}
                  rows={6}
                  placeholder="Revision instruction for the responsible agent…"
                  style={textInputStyle()}
                />
                <button
                  disabled={busy}
                  onClick={() =>
                    runAction(
                      () =>
                        returnAction({
                          issue_id: issueId,
                          target_agent_id: returnAgent || undefined,
                          instruction: returnInstruction || nextStepDraft,
                        }),
                      "Returned to agent.",
                    )
                  }
                  style={buttonStyle("#7c3aed", busy)}
                >
                  Return to agent
                </button>
              </div>
            ...
            <div style={panelStyle()}>
              ...
              <div style={{ display: "grid", gap: 10 }}>
                ...
                <button
                  ...
                  onClick={() =>
                    runAction(
                      ...
                      `Assigned to ${assignInput.trim()}.`,
                    )
                  }
                  style={buttonStyle("#6366f1", busy || !assignInput.trim())}
                >
                  Assign reviewer
                </button>
                <input
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Why this needs a higher-scope review…"
                  style={textInputStyle()}
                />
                <button
                  disabled={busy || !escalateNote.trim()}
                  onClick={() =>
                    runAction(
                      () =>
                        escalateAction({
                          issue_id: issueId,
                          comment: escalateNote,
                          escalate_to: escalateTo || undefined,
                        }),
                      "Escalated for review.",
                    )
                  }
                  style={buttonStyle("#f59e0b", busy || !escalateNote.trim())}
                >
                  Escalate
                </button>
              </div>
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ color: "#94a3b8", fontSize: 12 }}>
                      {entry.reviewerName} · {formatDate(entry.createdAt)}
                    </span>
                  </div>
                  {entry.comment ? (
                    <div
