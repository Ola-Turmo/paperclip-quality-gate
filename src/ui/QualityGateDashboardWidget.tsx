  ...
  if (!companyId) {
    return (
      <div style={widgetShell()}>
        Open within a company to see Quality Gate metrics.
      </div>
    );
  }
      ...
      >
        ...
        <div
          style={{
            fontSize: 24,
            fontWeight: 800,
            color: metricColor(summary?.averageDecisionScore ?? 0),
          }}
        >
          {summary?.averageDecisionScore ?? 0}/10
        </div>
        <div style={{ color: "#64748b", fontSize: 12, marginTop: 4 }}>
          {summary?.highRiskReviews ?? 0} high-risk packages |{" "}
          {summary?.unassignedPendingReviews ?? 0} unassigned pending
        </div>
      </div>
      ...
      {topItem ? (
        ...
        >
          ...
          <div style={{ color: "#94a3b8", fontSize: 13, marginTop: 4 }}>
            {topItem.issueId} | {topItem.headline}
          </div>
          <div style={{ color: "#94a3b8", fontSize: 12, marginTop: 8 }}>
            Risk: {topItem.topRiskLabel ?? "No active risk"}
          </div>
        </div>
      ) : null}
    </div>
...
function Metric({
  ...
  return (
    ...
    >
      <div
        style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase" }}
      >
        {label}
      </div>
      <div style={{ fontSize: 20, fontWeight: 800, color: tone, marginTop: 4 }}>
        {value}
      </div>
    </div>
  );
